...
SOURCE = os.path.join(REPO, "assets/models/uni/uni.glb")
...
def bind(mesh, rig):
    ...
    for v in mesh.data.vertices:
        ...
        # sorted, so that re-running the script writes the same bytes instead of reshuffling the
        # influences of every vertex that has two bones of equal pull
        weights = {n: 1.0 / (distances[n] + 1e-4) ** 3 for n in sorted(neighbours[closest])}
...
def leg_rests(rig):
    """Every leg's rest geometry, keyed by name."""
    return {name: leg_rest(rig, name) for name in LEGS}
...
def verify_walk(rig):
    ...
    ground = min(rest["hoof"].z for rest in leg_rests(rig).values())
...
def write_generated(rig):
    stride = stride_metres(rig)
    body = f"""// Generated by scripts/rig-uni.py from assets/models/uni/uni.glb — do not edit by hand.
...
def main():
    ...
    export(rig, mesh)
    write_generated(rig)
    print(json.dumps({"target": TARGET, "stride": round(stride_metres(rig), 4)}))
